...
export class HTTPServer implements DB {
  ...
  listen(): void {
    this.httpServer.listen(
      this.config.server_port || 9000,
      this.config.server_addr || "0.0.0.0"
    );
  }
  ...
  async query<T = any>(
    ...
    try {
      ...
      if (
        this.config.slow_query_threshold &&
        duration >= this.config.slow_query_threshold
      ) {
...
export class WebServer {
  ...
  constructor(readonly server: HTTPServer) {
    ...
    if (this.server.config.rate_limit) {
      ...
      const store = new RedisStore({
        ...
        prefix:
          "eosio-contract-api:" + server.connection.chain.name + ":rate-limit:",
      });
      ...
      this.limiter = rateLimit({
        ...
        skip: (req: express.Request) =>
          this.server.config.ip_whitelist.indexOf(req.ip) >= 0,
      ...
      if (this.server.config.rate_limit.bill_execution_time) {
        ...
        this.limiter = async (
          ...
          res.send = (data): express.Response => {
            (async (): Promise<void> => {
              const limitExceeded =
                Math.ceil((Date.now() - requestTime) / 1000) - 1;
    ...
    this.caching = expressRedisCache(
      ...
      "eosio-contract-api:" +
        this.server.connection.chain.name +
        ":express-cache:",
  ...
  private loadGitRevision(): void {
    try {
      const headRef = fs
        ...
        .replace(/\s/g, "");
      const revision = fs
        .readFileSync(`${__dirname}/../../.git/${headRef}`)
        .toString()
        .replace(/\s/g, "")
        .substring(0, 7);
      if (revision) {
  ...
  private routes(): void {
    ...
    async function buildHealthResponse(): Promise<any> {
      ...
      try {
        const query = await server.connection.database.query(
          "SELECT block_num FROM contract_readers"
        );
      ...
      try {
        if (!info) {
          ...
          Date.now() - 20 * 1000 <
          new Date(info.head_block_time + "+0000").getTime()
        ) {
          chainHealth = {
            status: "OK",
      ...
      return {
        ...
        data: {
          ...
          redis: {
            status:
              server.connection.redis.ioRedis.status === "ready"
                ? "OK"
                : "ERROR",
          },
          chain: chainHealth,
    ...
    router.get(
      ["/health", "/eosio-contract-api/health"],
    ...
    router.get(
      ["/alive", "/eosio-contract-api/alive"],
      async (_: express.Request, res: express.Response) => {
        ...
        if (health.data.chain.head_time <= Date.now() - 30 * 1000) {
          return res
            .status(500)
            .send(
              "chain api behind for " +
                (Date.now() - health.data.chain.head_time) / 1000 +
                " seconds"
            );
        }
        ...
        if (health.data.redis.status !== "OK") {
          return res
            .status(500)
            .send("redis state: " + health.data.redis.status);
        }

        if (health.data.postgres.status !== "OK") {
          return res
            .status(500)
            .send("postgres state: " + health.data.postgres.status);
        }
        ...
        for (const reader of health.data.postgres.readers) {
          if (reader.block_num <= health.data.chain.head_block - 180) {
            return res
              ...
              .send(
                "reader behind for " +
                  (health.data.chain.head_block - reader.block_num) +
                  " blocks"
              );
          }
        }
    ...
    router.get(
      ["/healthc", "/eosio-contract-api/healthc"],
      async (req, res) => {
    ...
    router.get(
      ["/timestamp", "/eosio-contract-api/timestamp"],
      async (_: express.Request, res: express.Response) => {
...
export class DocumentationServer {
  ...
  render(): void {
    ...
    router.get(
      "/docs",
      swagger.setup(this.documentation, {
